...
// e.g. singledepth 4:
// the 2nd banana is the 4th string, the enclosing element is ['banana', 'banana']
const multipleDepth = [['apple', ['apple']], 'orange', [[['banana', 'banana']]], 'pear', 'pear', ['peach']];
// --------------------------------FUNCTIONS---------------------------------------//
const mapper = (arr, start = null) =>
   arr.reduce((accum, el, idx) => {
     const index = start === null ? idx : start;
     return Array.isArray(el) ? accum.concat(mapper(el, index)) : accum.concat(index);
   }, []);
